...
Task 4 — Provision hosting: In progress. Jack created the Render account 2026-09-19 (the one step that had to be him). Plan corrected 2026-09-19: the Postgres add-on picked earlier was a mistake — it didn't match the design spec's own SQLite recommendation (§5.1) or the code Task 5 actually wrote (plain sqlite3). Now: Hobby workspace ($0) + paid web service ($7/mo) + a small persistent disk ($0.25/mo) ≈ $7.25/month, verified against Render's live docs/pricing. `deploy/render.yaml` and `deploy/.env.example` are written and committed; `api/db.py`/`db/migrate.py` now read the database path from a `DATABASE_PATH` env var so the database can live on that disk in production. Simulated the full deploy locally (built the DB outside the repo, ran the server, hit the endpoints including a real POST) — works. Still needs Jack to create the Blueprint in the Render dashboard himself (approving a paid plan is his call), then this task's Verify done check (hit the live URL from an outside network) can run.
...
**Jack needs to create the Render Blueprint next** (see Task 4 above) — that's the one remaining step Claude can't do (billing approval). After that, Task 6 (Document capture pipeline) is next in the build order.
...
Last updated: 2026-09-19 (Task 4 plan corrected, Task 5 completed)
