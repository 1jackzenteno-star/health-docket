...
Task 4 — Provision hosting: In progress. Jack created the Render account 2026-09-19 (the one step that had to be him). No web service or database created on Render yet — nothing to deploy until Task 5 exists. Chosen plan: Hobby workspace ($0) + paid web service ($7/mo) + paid Postgres ($6/mo) ≈ $13/month. Free tier was deliberately ruled out: Render's free Postgres expires 30 days after creation and is permanently deleted 14 days after that — wrong fit for data that needs to persist.

**Task 5 — Backend API is next.** Backend/Frontend phase, needs only Task 1 (Done) — not blocked by Task 4 being incomplete. Seven endpoints, spec'd in full in §6 of the design doc. Once Task 5's code exists, finishing Task 4 (creating the actual Render web service + Postgres, writing deploy/render.yaml, deploying, and running Task 4's Verify done check — hit the live URL from an outside network) becomes possible for the first time, so the two tasks will likely close out together rather than strictly in order.
